...
func Scan() ([]scan.CategoryResult, error) {
	...
	// User App Caches
	if cr, err := scanTopLevel(filepath.Join(home, "Library", "Caches"), "system-caches", "User App Caches"); err == nil && cr != nil {
		results = append(results, *cr)
	}

	// User Logs
	if cr, err := scanTopLevel(filepath.Join(home, "Library", "Logs"), "system-logs", "User Logs"); err == nil && cr != nil {
		results = append(results, *cr)
	}
